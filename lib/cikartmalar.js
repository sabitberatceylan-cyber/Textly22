  ...
  try {
    ...
    try {
      ...
      if (intentSonuc && intentSonuc.resultCode === -1) {
        hamIntentMetni = typeof intentSonuc.data === 'string' ? intentSonuc.data : '';
        secilenUri = uriTemizle(intentSonuc.data) || uriTemizle(intentSonuc.extra);
    ...
    const hamLower = (hamIntentMetni || '').toLowerCase();
    const zipIhtimali = hamLower.includes('application/zip') || hamLower.includes('.zip') || uriCozulmus.includes('.zip');

    let dosyaUzantisi = zipIhtimali ? 'zip' : 'bin';
    if (!zipIhtimali) {
      if (uriCozulmus.includes('.webp') || hamLower.includes('webp')) dosyaUzantisi = 'webp';
      else if (uriCozulmus.includes('.png') || hamLower.includes('png')) dosyaUzantisi = 'png';
    ...
    const yerelVarMi = (await FileSystem.getInfoAsync(yerelGeciciYol).catch(() => ({ exists: false }))).exists;
    const okunacakYol = yerelVarMi ? yerelGeciciYol : secilenUri;

    // Dosyanın base64 içeriğini oku
    let dosyaBase64 = null;
    try {
      dosyaBase64 = await FileSystem.readAsStringAsync(okunacakYol, {
        encoding: FileSystem.EncodingType.Base64,
      });
    } catch (okumaHatasi) {
      // Eğer kopyalanan yoldan okunamadıysa doğrudan secilenUri'den dene
      if (okunacakYol !== secilenUri) {
        try {
    ...
    if (!dosyaBase64) {
      return { tamam: false, hata: 'Seçilen dosya açılamadı veya erişim izni yok.' };
    }

    // ZIP dosyası kontrolü (PK magic bytes: base64 'UEs' veya uzantı / intent zip)
    const zipMi = dosyaBase64.startsWith('UEs') || zipIhtimali || dosyaUzantisi === 'zip';
