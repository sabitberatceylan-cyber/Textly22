...
export async function cokluGorselIsleVeYukle(sunucuAdres, kullanici, sifre, varliklar, onIlerleme) {
  ...
  for (let i = 0; i < toplam; i++) {
    ...
    try {
      const uriKucuk = (varlik.uri || varlik.fileName || '').toLowerCase();
      const mimeKucuk = (varlik.mimeType || varlik.type || '').toLowerCase();
      const videoMu = uriKucuk.endsWith('.mp4') || uriKucuk.endsWith('.webm') || mimeKucuk.startsWith('video/') || varlik.type === 'video';
      const gifMi = uriKucuk.endsWith('.gif') || mimeKucuk === 'image/gif';
      const webpMi = uriKucuk.endsWith('.webp') || mimeKucuk === 'image/webp';

      let yuklenecekUri = varlik.uri;
      let mimeTuru = 'image/png';
      let cikartmaTuru = 'resim';

      if (videoMu) {
        mimeTuru = uriKucuk.endsWith('.webm') ? 'video/webm' : 'video/mp4';
        cikartmaTuru = 'video';
      } else if (gifMi) {
        mimeTuru = 'image/gif';
        cikartmaTuru = 'gif';
        ...
        // PNG/JPEG için 512x512 standardına getir
        const manipule = await ImageManipulator.manipulateAsync(
          varlik.uri,
          [{ resize: { width: 512 } }],
          { compress: 0.85, format: ImageManipulator.SaveFormat.PNG }
        );
        yuklenecekUri = manipule.uri;
      }

      // Sunucuya yükle
      const yukle = await dosyaYukleDirekt(sunucuAdres, kullanici, sifre, yuklenecekUri, mimeTuru);
  ...
  try {
    ...
    if (zipMi) {
      ...
      const icerikler = Object.keys(zip.files).filter((dosyaAdi) => {
        ...
        return /\.(png|webp|jpg|jpeg|gif|mp4|webm)$/i.test(dosyaAdi);
      });

      if (icerikler.length === 0) {
        return { tamam: false, hata: 'Seçilen ZIP dosyasında uygun çıkartma (WebP/GIF/PNG/Video) bulunamadı.' };
      }
      ...
      for (let i = 0; i < toplam; i++) {
        ...
        try {
          ...
          const dosyaAdiKucuk = ad.toLowerCase();
          let uzanti = 'png';
          let mimeTuru = 'image/png';
          let cikartmaTuru = 'resim';

          if (dosyaAdiKucuk.endsWith('.webp')) {
            uzanti = 'webp';
            mimeTuru = 'image/webp';
            cikartmaTuru = 'webp';
          } else if (dosyaAdiKucuk.endsWith('.gif')) {
            uzanti = 'gif';
            mimeTuru = 'image/gif';
            cikartmaTuru = 'gif';
          } else if (dosyaAdiKucuk.endsWith('.mp4')) {
            uzanti = 'mp4';
            mimeTuru = 'video/mp4';
            cikartmaTuru = 'video';
          } else if (dosyaAdiKucuk.endsWith('.webm')) {
            uzanti = 'webm';
            mimeTuru = 'video/webm';
            cikartmaTuru = 'video';
          }
          ...
          let yuklenecekUri = tempYol;
          // Sadece statik PNG/JPEG dosyalarını ImageManipulator ile 512x512 yap.
          // WebP, GIF ve videolara dokunma ki animasyon kareleri ve hareket kaybolmasın!
          if (uzanti === 'png' || dosyaAdiKucuk.endsWith('.jpg') || dosyaAdiKucuk.endsWith('.jpeg')) {
            const manipule = await ImageManipulator.manipulateAsync(
              tempYol,
              [{ resize: { width: 512 } }],
              { compress: 0.85, format: ImageManipulator.SaveFormat.PNG }
      ...
      if (secilenUri && secilenUri.startsWith(FileSystem.cacheDirectory)) {
        FileSystem.deleteAsync(secilenUri, { idempotent: true }).catch(() => {});
      }
    ...
    // Tekil dosya (.webp veya görsel/video) seçilmişse
    if (onIlerleme) onIlerleme({ durum: 'yukleniyor', metin: 'Çıkartma hazırlanıyor...' });
    let tekilUzanti = 'png';
    let tekilMime = 'image/png';
    let tekilTur = 'resim';

    if (uriLower.includes('.webp') || dosyaBase64.startsWith('UklGR')) {
      tekilUzanti = 'webp';
      tekilMime = 'image/webp';
      tekilTur = 'webp';
    } else if (uriLower.includes('.gif') || dosyaBase64.startsWith('R0lGOD')) {
      tekilUzanti = 'gif';
      tekilMime = 'image/gif';
      tekilTur = 'gif';
    ...
    if (tekilUzanti === 'png') {
      ...
      tekilYuklenecek = manipule.uri;
    }

    const yukle = await dosyaYukleDirekt(sunucuAdres, kullanici, sifre, tekilYuklenecek, tekilMime);
    // Geçici dosyaları temizle
