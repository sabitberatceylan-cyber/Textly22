...
async function govdeliIstek(url, gövde, zamanAsimiMs = 7500) {
...
async function getIstek(url, zamanAsimiMs = 7500) {
  const controller = new AbortController();
  const zamanlayici = setTimeout(() => controller.abort(), zamanAsimiMs);
  try {
...
export async function gruplariGetir(sunucuAdres, kullanici, sifre) {
  ...
  if (!sonuc.tamam) return sonuc;
  const liste = Array.isArray(sonuc.veri) ? sonuc.veri : (Array.isArray(sonuc.liste) ? sonuc.liste : []);
...
export function medyaAdresi(sunucuAdres, kullanici, sifre, medyaUrl) {
  if (!medyaUrl || typeof medyaUrl !== 'string') return '';
  if (medyaUrl.startsWith('file://')) return medyaUrl;
  let temizAdres = sunucuAdres || '';
  if (temizAdres.endsWith('/')) temizAdres = temizAdres.slice(0, -1);
  let tam = medyaUrl;
  if (!medyaUrl.startsWith('http://') && !medyaUrl.startsWith('https://')) {
    const yol = medyaUrl.startsWith('/') ? medyaUrl : `/${medyaUrl}`;
    tam = `${temizAdres}${yol}`;
  }
  if (!tam.includes('kullanici=') && kullanici) {
    const ayrac = tam.includes('?') ? '&' : '?';
    tam = `${tam}${ayrac}kullanici=${encodeURIComponent(kullanici)}&sifre=${encodeURIComponent(sifre || '')}`;
  }
  return tam;
}

export async function grupBilgiGetir(sunucuAdres, kullanici, sifre, grupId) {
  if (!grupId) return { tamam: false, hata: 'Grup ID eksik' };
  const sonuc = await getIstek(
    `${sunucuAdres}/grup-bilgi?grupId=${encodeURIComponent(grupId)}&kullanici=${encodeURIComponent(kullanici)}&sifre=${encodeURIComponent(sifre)}`
  );
  if (!sonuc.tamam) return sonuc;
  return { tamam: true, grup: sonuc.veri?.grup || sonuc.veri };
}
