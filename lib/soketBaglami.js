...
export function SoketSaglayici({ sunucuAdres, kullanici, sifre, children }) {
  ...
  const gecmisiIcinYukle = useCallback((anahtar, gelenListe) => {
    setMesajDeposu((mevcut) => {
      ...
      const gelenHarita = new Map((gelenListe || []).map((m) => [m.id, m]));
  ...
  const gruplariAyarla = useCallback((gelenGruplar) => {
    ...
    setGruplar((mevcut) => {
      const map = new Map();
      // Önce gelen sunucu gruplarını ekle
      gelenGruplar.forEach((g) => {
        if (g && g.id) map.set(String(g.id), g);
      });
      // Mevcut yerel veya önbellek gruplarını koru (sunucudan silinmemişse veya henüz dönmemişse)
      (mevcut || []).forEach((g) => {
        if (g && g.id) {
          const varOlan = map.get(String(g.id));
          if (!varOlan) {
            map.set(String(g.id), g);
          } else {
            map.set(String(g.id), {
              ...varOlan,
              resimUrl: varOlan.resimUrl || g.resimUrl,
              aciklama: varOlan.aciklama || g.aciklama,
            });
          }
        }
      });
      const birlesmis = Array.from(map.values());
      AsyncStorage.setItem(`@textly_gruplar_cache_${kullanici}`, JSON.stringify(birlesmis)).catch(() => {});
      ...
      setGrupBilgileri((eski) => ({ ...eski, ...bil }));
