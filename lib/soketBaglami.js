...
export function SoketSaglayici({ sunucuAdres, kullanici, sifre, children }) {
  ...
  const gruplariAyarla = useCallback((gelenGruplar) => {
    ...
    setGruplar((mevcut) => {
      const guncelMap = new Map(map);
      (mevcut || []).forEach((g) => {
        if (g && g.id) {
          const varOlan = guncelMap.get(String(g.id));
          if (!varOlan) {
            guncelMap.set(String(g.id), g);
          } else {
            guncelMap.set(String(g.id), {
              ...varOlan,
      ...
      const birlesmis = Array.from(guncelMap.values());
